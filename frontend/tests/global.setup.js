...
export default async () => {
	...
	await popup.fill('input[type="email"]', email, { timeout: 60000 });
	await popup.click('button:has-text("Next")');

	await popup.fill('input[type="password"]', password, { timeout: 60000 });
	await popup.click('button:has-text("Next")');

	const allowButton = popup.locator('button:has-text("Allow")');
	if (await allowButton.isVisible({ timeout: 60000 })) {
